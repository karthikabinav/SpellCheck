...
public class Execute
{
	...
	public static String misspeltString;
	...
	private static BigInteger bigInt(int lDnoConfusion)
	{
		return new BigInteger ( Integer.toString(lDnoConfusion));
	}
	...
	public static void normalizeTuple(ArrayList<Tuple> corrections)
	{
		...
		BigInteger confusion_rank = BigInteger.ONE;
		...
		for (Tuple cand : corrections)
		{
			...
			if (cand.confusion_rank.compareTo(confusion_rank) > 0)
			{
				confusion_rank = cand.confusion_rank;
			}
		}
		for (Tuple cand : corrections)
		{
			...
			cand.trigram_rank = cand.trigram_rank.multiply(long_max).divide(
					trigram_rank);
			cand.confusion_rank = BigInteger.valueOf(2).multiply(long_max
					.divide(cand.confusion_rank));
		}
		for (Tuple cand : corrections)
		{
			cand.overall_rank = cand.trigram_rank.add(
					new BigInteger(Long.toString(cand.rank))).add(
					(cand.confusion_rank));
		}
